...
"$PYTHON" paper_broker.py sync            >> "$LOG_FILE" 2>&1 || true   # entry+exit fills → paper_fills.json
"$PYTHON" paper_broker.py intraday-report >> "$LOG_FILE" 2>&1 || true   # today's intraday round trips
"$PYTHON" validate.py run                 >> "$LOG_FILE" 2>&1           # snapshot + mark + report (swing)
